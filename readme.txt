...
- **Jadwal Sholat** — `/sholat` lihat jadwal hari ini, `/sholat 15` jadwal 15 hari ke depan
- **Arah Kiblat** — `/sholat-kiblat` arah kiblat dari lokasi yang dikonfigurasi
- **Auto-Reminder** — Otomatis kirim pesan reminder 10 menit sebelum waktu sholat via Gemini (tsundere-style)
- **Role Mention** — User assign role `@Sholat` untuk dapat notifikasi
- **Dual API** — myQuran (Kementerian Agama RI) sebagai primary, AlAdhan sebagai fallback
- **City Search** — `/sholat-set-city` cari dan set kota dari 517 kota di Indonesia
- **Configurable** — Channel, role, kota, dan timing bisa diatur via slash command
...
| `/sholat` | Lihat jadwal sholat hari ini (atau N hari ke depan) |
| `/sholat-kiblat` | Lihat arah kiblat dari lokasi kota |
| `/sholat-status` | Cek konfigurasi sholat saat ini |
| `/sholat-set-channel` | Set channel untuk auto-reminder sholat (admin) |
| `/sholat-set-role` | Set role untuk mention di reminder sholat (admin) |
| `/sholat-set-city` | Set kota untuk jadwal sholat (admin) |
...
SHOLAT_ENABLED=true
SHOLAT_CITY_ID=
SHOLAT_CITY_NAME=KOTA JAKARTA
SHOLAT_LAT=-6.2088
SHOLAT_LNG=106.8456
SHOLAT_METHOD=8
SHOLAT_TIMEZONE=Asia/Jakarta
SHOLAT_ROLE_ID=0
SHOLAT_REMINDER_MINUTES=10
```
...
│   └── sholat.py            # Jadwal Sholat & Auto-Reminder
├── core/                    # Bot core logic
...
│   ├── sholat_client.py     # Jadwal Sholat API (myQuran + AlAdhan) + AI reminder
